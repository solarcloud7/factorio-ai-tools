...
uv run python -m factorio_ai_tools.ingest.ingest_factorio      # -> data/factorio_lancedb  (Lua API + prototype docs, pinned versions 1.1.110 + 2.0.76 + 2.1.8)
...
- `data/factorio_lancedb` → table `docs`: `text`, `class_name`, `version`, `url`, `node_type`, `returns`, `source_url`, `content_hash`. Holds **three pinned versions** `common.SUPPORTED_FACTORIO_VERSIONS = ("1.1.110", "2.0.76", "2.1.8")` (1.1 legacy, 2.0.76 stable, 2.1.8 experimental — what mod creators target; no moving `latest` — it drifted and was removed); `search_factorio_docs` **requires** a concrete `version` (one of those) plus optional `class_name`. FTS index on `text`. Writes `version.txt`.
...
- `data/prototypes_lancedb` → table `prototypes`: `prototype_type`, `prototype_name`, `category`, `content`, `version`, `content_hash`. One **structured** record per Factorio prototype (recipes/items/entities/technologies/quality/planets) **per version** holding exact numerical values; `search_factorio_prototypes` **requires** a concrete `factorio_version` (`common.SUPPORTED_PROTOTYPE_VERSIONS = ("2.0.76", "2.1.8")` — a subset of the docs versions; 1.1 has no practical `--dump-data`) and optionally filters by `prototype_type` (umbrella values `item`/`entity` expand to their raw subtypes — `module`, `ammo`, `furnace`, …). **Multi-version:** the merge-insert dedup key and orphan-pruning are `(prototype_type, prototype_name, version)`-scoped, so re-ingesting one version never deletes another's rows. FTS index on `content`. Writes a comma-joined `version.txt` of all versions present. Built from Factorio's own **`factorio --dump-data`** JSON export (the fully-resolved `data.raw`); `ingest_prototypes.py` ingests `FACTORIO_DATA_DUMP` (a single dump) or, by default, **every** `factorio-export/vanilla_<ver>/data-raw-dump.json` present — **no Lua parsing, no `factorio-data` clone**. This is the **vanilla baseline** (base + official DLC); modded games change `data.raw`, so a modded game differs. Built locally from the dumps (the maintainer produces one per Factorio version), then **shipped in the release zip** like the other stores — it's in `RELEASE_STORES` (server.py) and `STORES` (Makefile).

**Adding a store (touchpoint checklist).** When you add another store, update **every** one of these — PR #9 (the prototypes store) missed several, and a code review caught it only after the fact; the doc-drift + contract tests now fail if you skip the doc ones: (1) the ingest script under `ingest/` (built on `common`); (2) the table handle + `search_*` tool in `server.py`; (3) this **per-store table**, plus the store count in the Overview / REPL-note / "All N scripts import `common`" lines above; (4) `README.md`'s pipeline list; (5) `docs/` — `README.md`, `architecture.md`, the `tools.md` tool table; (6) the `Makefile` — an `ingest-<x>` target + `ingest-all`, and `STORES` **only if** the store ships in the release zip; (7) tests — `tests/test_ingest_<x>.py` and the `CONTRACT` map in `tests/test_server_contract.py`; (8) if it clones a repo, add that dir to `.gitignore`. **Decide explicitly** whether it joins `RELEASE_STORES` (shipped in `factorio_lancedb.zip`) or stays local-only — and keep `RELEASE_STORES` (server.py) and `STORES` (Makefile) in lockstep: a store in the missing-check list but not in the zip causes an infinite re-download loop.
...
- **Manual build & deploy of the databases.** Ingestion is additive/idempotent, so iterate locally with `make ingest-all` (or individual `ingest_*` runs), then ship the final full build with `make deploy-dbs`: it runs `make compact`, zips the **six `data/*_lancedb` stores** into `factorio_lancedb.zip`, and `gh release upload`s it to the latest release. (`prototypes_lancedb` must be built — `make ingest-prototypes` from a `--dump-data` export — before packaging.) The artifact name `factorio_lancedb.zip` is load-bearing — `server.py`'s `ensure_databases()` downloads exactly that asset from `releases/latest/download/` and only short-circuits when **all six** release store dirs are already present.
