...
uv run python -m factorio_ai_tools.ingest.ingest_prototypes    # -> data/prototypes_lancedb (vanilla prototype values; needs the factorio-data clone — `make ingest-prototypes` clones it)

# Or build all six at once (clones Clusterio + factorio-data + the configured generic repos as needed):
make ingest-all
