...
- **Existing installs do not auto-refresh data.** `ensure_databases` downloads only *missing* release stores, so an install that already has all five release stores keeps its local `data/` until a store directory is deleted. (`prototypes_lancedb` is built locally and not part of the release zip.)
