...
- **Existing installs do not auto-refresh data.** `ensure_databases` downloads only *missing* stores, so an install that already has all five keeps its local `data/` until a store directory is deleted.
